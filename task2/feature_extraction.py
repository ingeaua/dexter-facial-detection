...
def get_positive_character_hog(character, dim_window=36, dim_hog_cell=6, augument=False):

    characters_files = ['dad', 'deedee', 'dexter', 'mom']
    positive_descriptors = []

    for i in trange(len(characters_files)):

        character_folder = characters_files[i]
        images_path = os.path.join('../../antrenare/' + character_folder + '/', '*.jpg')
        bounding_boxes = np.loadtxt('../../antrenare/' + character_folder + '_annotations.txt', dtype='str')
...
def get_negative_character_hog(character, dim_window=36, dim_hog_cell=6, neg_per_image=6, min_patch_size=32, max_patch_size=120, max_iou=0.25):
    ...
    for i in trange(len(characters_files)):
        ...
        character_folder = characters_files[i]
        images_path = '../../antrenare/' + character_folder + '/*.jpg'
    ...
    for charac in characters_files:
        if charac != character:
            pos_desc = get_positive_character_hog(charac, dim_window, dim_hog_cell, True)
            negative_descriptors = np.concatenate((negative_descriptors, pos_desc), axis=0)
            print(f'{charac} pos: {pos_desc.shape}')
